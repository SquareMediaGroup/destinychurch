  ...
  return (
    <div className={`${cardClass} mb-6 p-5`}>
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="text-sm font-black uppercase tracking-wider text-destiny-grey/60">
          Ask the log
        </h2>
      </div>
      ...
      >
        <input
          ...
          className="w-full flex-1 rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm text-destiny-grey outline-none transition placeholder:text-destiny-grey/30 focus:border-destiny-orange/50 focus:ring-2 focus:ring-destiny-orange/15"
        />
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {EXAMPLES.map((example) => (
            <button
              ...
              type="button"
              onClick={() => ask(example)}
              className="rounded-full border border-black/10 bg-white px-3 py-1.5 text-xs font-bold text-destiny-grey/55 transition hover:bg-[#f5f7fa] hover:text-destiny-grey"
            >
              {example}
            </button>
          ))}
        ...
        <div className="mt-4">
          <div className="rounded-2xl bg-[#f5f7fa] p-4">
            {answer.split("\n").filter(Boolean).map((line, i) => (
              <p
                key={i}
                className="mb-2 text-sm leading-relaxed text-destiny-grey last:mb-0"
              >
                {line}
              </p>
            ))}
          ...
          {entries.length > 0 && (
            <div className="mt-3">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-destiny-grey/40">
              ...
              <div className="flex flex-col gap-1">
                {entries.map((entry) => (
                  <button
                    ...
                    className="flex w-full flex-wrap items-center gap-2 rounded-xl border border-black/5 px-3 py-2 text-left transition hover:bg-[#f5f7fa]"
                  >
                    <Badge tone={actionTone(entry.action)}>{actionLabel(entry.action)}</Badge>
                    <span className="min-w-0 flex-1 truncate text-sm text-destiny-grey">
                      {entry.summary}
                    </span>
                    <span className="shrink-0 text-xs text-destiny-grey/40">
                      {entry.actor_email ?? "System"} · {relativeTime(entry.created_at)}
                    </span>
                    <span className="shrink-0 text-xs text-destiny-grey/30">
          ...
          <button
            ...
            className="mt-3 text-xs font-bold text-destiny-grey/45 transition hover:text-destiny-grey"
          >
            Ask something else
          </button>
        </div>
