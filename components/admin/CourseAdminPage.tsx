...
// The one implementation behind /admin/alpha, /admin/recovery,
// /admin/bible-course and /admin/cap-money. Everything that varies between
// them — which event types the page owns, its accent, its wording — comes from
// COURSE_ADMIN_PAGES in lib/courseEvents.ts, so a new course is a registry
// entry plus a four-line route file.
//
// Colour is applied as inline rgba() derived from the accent rather than
// Tailwind opacity classes, because the accent is per-course data and cannot
// be a compile-time class name.
...
export default function CourseAdminPage({ page }: { page: CourseAdminPageSlug }) {
  const config = COURSE_ADMIN_PAGES[page];
  const types = config.types as readonly CourseEventType[];
  ...
  // Form state
  const [type, setType] = useState<CourseEventType>(types[0]);
  ...
  // One independent site_banner row per type; several can be live at once.
  const [bannerActive, setBannerActive] = useState<Partial<Record<CourseEventType, boolean>>>({});
  const [bannerSaving, setBannerSaving] = useState<Partial<Record<CourseEventType, boolean>>>({});

  const fetchBanners = useCallback(async () => {
    try {
      const results = await Promise.all(
        types.map(async (t) => {
          const res = await fetch(`/api/admin/banner?type=${t}`).then((r) =>
            r.json(),
          );
          return [t, !!res?.active && res?.type === t] as const;
        }),
      );
      setBannerActive(Object.fromEntries(results));
    } catch {
      // A banner read failing should not blank the page; the toggles just
      // show as off until the next load.
    }
  }, [types]);
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      setEvents(
        Array.isArray(data)
          ? data.filter((e: CourseEvent) => types.includes(e.type))
          : [],
      );
  ...
  async function toggleBanner(t: CourseEventType, next: boolean) {
    const meta = COURSE_EVENT_META[t];
    setBannerSaving((prev) => ({ ...prev, [t]: true }));
    try {
    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      <div className="max-w-4xl">
        ...
        <div className="mb-8 rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
          ...
          <div className={multi ? "grid gap-3 sm:grid-cols-2" : undefined}>
            {types.map((t) => (
              <BannerCard
                key={t}
                label={COURSE_EVENT_META[t].label}
                hint={COURSE_EVENT_META[t].hint}
                active={!!bannerActive[t]}
                saving={!!bannerSaving[t]}
                onToggle={() => toggleBanner(t, !bannerActive[t])}
                accent={accent}
                accentRgb={accentRgb}
              />
            ))}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {multi && (
                <div>
                  ...
                  <select
                    value={type}
                    onChange={(e) => setType(e.target.value as CourseEventType)}
                    className={inputClass}
                  >
                    {types.map((t) => (
                      <option key={t} value={t}>
                        {COURSE_EVENT_META[t].label}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <select
                    value={frequency}
                    onChange={(e) => setFrequency(e.target.value as AlphaFrequency)}
                    className={inputClass}
                  >
        ...
        {types.map((t) => (
          <EventSection
            key={t}
            title={COURSE_EVENT_META[t].eventsLabel}
  ...
  return (
    <div
      className={`rounded-xl border px-4 py-3 transition ${
        active ? "" : "border-black/8 bg-black/[0.02]"
      }`}
      style={
        active
          ? {
...
function EventSection({
  ...
  events: CourseEvent[];
  loading: boolean;
  onToggle: (event: CourseEvent) => void;
    ...
    <div className="mb-8 overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm">
      ...
      {loading ? (
        ...
        <div className="divide-y divide-black/5">
          {events.map((event) => {
            const formatted = new Date(event.start_date).toLocaleDateString(
              "en-GB",
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    ...
                    style={event.active ? { backgroundColor: accent } : undefined}
                  >
                    <span
