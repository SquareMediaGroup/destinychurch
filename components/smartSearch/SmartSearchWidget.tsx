...
// The Smart Search pill + panel — exact same UI and behaviour everywhere it
// appears. Shared by the site-wide floating pill (FloatingSmartSearch, fixed
// to the viewport) and the 404 page (NotFoundSearch, dropped inline in the
// page flow instead of floating). Only the outer wrapper's positioning
// differs between the two; everything inside is identical.
...
export default function SmartSearchWidget({
  ...
  /** True (default): float fixed to the bottom of the viewport, as on every
   *  normal page. False: sit inline wherever it's rendered (the 404 page). */
  fixed?: boolean;
}) {
  ...
  const outerClassName = fixed
    ? "pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center px-4 pb-[max(1rem,env(safe-area-inset-bottom))] transition-[bottom] duration-300 ease-out"
    : "flex justify-center";
  const outerStyle = fixed ? { bottom: "var(--podcast-dock-height, 0px)" } : undefined;
