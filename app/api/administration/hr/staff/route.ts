...
import { createLogin, rolesByUserId, sanitizeRoles } from "@/lib/staffLogins";
...
export async function GET() {
  ...
  // Enrich each row with the linked auth user's roles so the directory and
  // edit modal can show / prefill backend access.
  const roleMap = await rolesByUserId(supabase);
  const enriched = (data ?? []).map((row) => {
    const login = row.auth_user_id ? roleMap.get(row.auth_user_id) : undefined;
    return {
      ...row,
