...
- Sermon streaming and full-archive browsing — series, month, and guest-speaker filters with fuzzy search (speaker is shown on each sermon but is no longer a filter facet), a Watch/Listen switch on any paired sermon (podcast audio that keeps playing as you navigate the site), and a live-stream banner
...
- Staff self-service portal (`/portal`) — a separate login surface, now a personal dashboard where staff see their profile and account settings (avatar/email/password), a team directory, their reviews and onboarding, request leave, download documents, and submit and track their design requests (an `hr_admin` access level runs HR without full super-admin)
...
├── portal/           # Staff self-service dashboard (profile, team, reviews, leave, documents, design requests)
