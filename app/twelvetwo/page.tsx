...
const NAVY = COURSE_ADMIN_PAGES.recovery.accent;
...
export default function DestinyTwelveTwoPage() {
  ...
  const { events, loading } = useCourseEvents((e) => e.type === "recovery");
    ...
    <>
      ...
      {events.length > 0 && (
        <div className={`mx-auto max-w-5xl px-4 pb-14 lg:px-8 ${events.length > 1 ? "space-y-6" : ""}`}>
          {events.map((event) => (
            <CourseEventCard key={event.id} event={event} accentColor={NAVY} />
          ))}
        </div>
      )}
