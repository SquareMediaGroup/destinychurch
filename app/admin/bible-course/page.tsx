...
const ACCENT = "#1b4965";
const ACCENT_RGB = "27,73,101";

export default function BibleCourseAdminPage() {
  ...
  const fetchBanner = useCallback(async () => {
    try {
      const res = await fetch("/api/admin/banner?type=bible_course").then((r) =>
        r.json()
      );
      setBannerActive(!!res?.active && res?.type === "bible_course");
    } catch {
      // ignore
    }
  }, []);
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const bibleCourse = Array.isArray(data)
        ? data.filter((e: BibleCourseEvent) => e.type === "bible_course")
        : [];
      setEvents(bibleCourse);
  ...
  async function handleToggle(event: BibleCourseEvent) {
      ...
      <div className="max-w-4xl">
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-[10px] text-destiny-grey/50">
                  8-session course exploring the Bible
                </p>
              </div>
              <button
                type="button"
        ...
        <div className="mb-8 overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm">
          <div className="border-b border-black/5 px-6 py-4">
            <p className="text-sm font-bold text-destiny-grey">
              Bible Course Events ({events.length})
            </p>
          </div>
          {loading ? (
