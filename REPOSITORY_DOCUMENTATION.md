...
CREATE TABLE admin_roles (
  ...
  safeguarding_admin boolean NOT NULL DEFAULT false,  -- /api/admin/destiny-one — reports, frozen groups, audited transcripts
...
**Tested by:** `scripts/test-sql.sh` (`npm run test:sql`) — applies Supabase stubs + this migration to
a throwaway local Postgres 16 and runs `tests/sql/destiny-one.sql` (51 checks).
...
```typescript
// Daily at 03:30. Bearer CRON_SECRET (fails closed). Re-reads every linked Destiny One
// member from ChurchSuite (name, adult_on, still active?) via resyncMember(); someone removed
// from ChurchSuite drops to `pending`, which freezes any group left with < 2 adults. A
// ChurchSuite outage changes nothing. Then d1_reconcile_all() re-checks every live group.
```
...
applies three gates — signed in → `active` member linked to ChurchSuite (`not_verified` otherwise) →
...
| Route | Methods | Notes |
|---|---|---|
| `auth/link` | POST | After every sign-in: match to ChurchSuite (`linkMember`), return `D1Me` |
...
#### Destiny One safeguarding API (`/api/admin/destiny-one/*`)
Gated to `safeguarding_admin` (middleware ROUTE_RULES, and re-checked in each route by
`requireSafeguardingAdmin`). No admin UI yet — API only.
- `GET events?open=1`, `PATCH events/[id]` — the freeze/report queue; mark handled.
...
- `POST groups/[id]/freeze` — manual freeze / lift (a manual freeze isn't lifted by the automatic rule).
- `GET members?status=pending`, `PATCH members/[id]` — link an account to a ChurchSuite contact/child
  by hand (shared family emails, children, mismatches), suspend/reinstate, set leader roles.
...
share token is ever emailed), the new-request alert, claimed, delivered, changes
...
Resend reject the whole send. Two rules hold throughout: every send is
...
- `churchsuite.server.ts` — ChurchSuite API v2: client-credentials token (cached), contact/child
  ...
  it as "no change".
- `identity.server.ts` — `linkMember` (every uncertain case → `pending`; outages never downgrade),
  `resyncMember` (nightly).
...
Access levels live in `lib/adminRoles.ts` + the `admin_roles` table — ten
independent per-user booleans (`training_admin`, `event_admin`, `store_admin`,
`site_admin`, `host`, `hr_admin`, `design_admin`, `sermon_admin`, `safeguarding_admin`, `super_admin`; see [admin_roles](#10b-admin_roles)). Auth *and*
...
email one-time code (Supabase OTP) and Sign in with ChurchSuite (staff/leaders; see the API section).
Signing in is not enough to chat: `linkMember` must tie the account to exactly one active ChurchSuite
record (real name + `adult_on` come from there), and the member must accept the current privacy /
terms / chat-review notices. Leader roles (`group_leader`, `senior_leadership`) are set by a
safeguarding admin. Supabase's phone auth provider should be **disabled**; the database refuses to
activate an account with a phone number regardless.
