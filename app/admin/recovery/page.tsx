...
export default function RecoveryAdminPage() {
  ...
  const fetchBanner = useCallback(async () => {
    try {
      const res = await fetch("/api/admin/banner?type=recovery").then((r) =>
        r.json()
      );
      setBannerActive(!!res?.active && res?.type === "recovery");
    } catch {
      // ignore
    }
  }, []);
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const recovery = Array.isArray(data)
        ? data.filter((e: RecoveryEvent) => e.type === "recovery")
        : [];
      setEvents(recovery);
    ...
    try {
      const res = await fetch("/api/admin/banner", {
        ...
        body: JSON.stringify({
          active: next,
          type: "recovery",
          message: "Destiny Recovery",
          link: "/destiny-recovery",
  ...
  async function handleToggle(event: RecoveryEvent) {
        ...
        <div className="mb-8 rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
          ...
          <div
            className={`rounded-xl border px-4 py-3 transition ${
              bannerActive ? "bg-[#006756]/5" : "border-black/8 bg-black/[0.02]"
            }`}
            style={
              bannerActive
                ? { borderColor: "rgba(0,103,86,0.3)" }
                : undefined
            }
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-bold text-destiny-grey">Recovery</p>
                <p className="text-[10px] text-destiny-grey/50">
                  12-step Christian recovery course
                </p>
              </div>
              <button
                type="button"
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            ...
            {format === "in_person" ? (
              ...
              <div
                ...
                style={{
                  borderColor: "rgba(0,103,86,0.3)",
                  backgroundColor: "rgba(0,103,86,0.04)",
                }}
              >
                        ...
                        {isOnline ? (
                          ...
                          event.location && (
                            <span
                              ...
                              style={{
                                backgroundColor: "rgba(0,103,86,0.1)",
                                color: ACCENT,
                              }}
                            >
                              {event.location}
